...
class Generate
{
    ...
    public function run($data, $option = 'all')
    {
        ...
        if ($action != 'buildDir') {
            // 文件路径
            $pathView = APP_PATH . $this->module . DS . "view" . DS . $this->dir . $this->nameLower . DS;//视图
            $pathTemplate = APP_PATH . $this->module . DS . "view" . DS . "generate" . DS . "template" . DS;//模板
            $fileName = APP_PATH . $this->module . DS . "%NAME%" . DS . $this->dir . $this->name . ".php";//php文件
            $code = $this->parseCode();
            // 执行方法
